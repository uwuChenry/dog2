...
# Pre-defined configs
##
from .dog1ArticulationCFG import MY_DOG_CFG  # Import your dog1 articulation config
...
@configclass
class Dog2SceneCfg(InteractiveSceneCfg):
    """Configuration for the terrain scene with a dog1 robot."""
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityCommandCfg(
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(-2.0, 2.5), lin_vel_y=(-1.0, 1.0), ang_vel_z=(-1.0, 1.0), heading=(-math.pi, math.pi)
        ),
    )
...
class EventCfg:
    ...
    reset_robot_joints = EventTerm(
        ...
        mode="reset",
        params={
            "position_range": (0.5, 1.5), #0.5 1.5
...
class TerminationsCfg:
    ...
    # Contact-based termination commented out for now
...
class CurriculumCfg:
    ...
    # No terrain curriculum for flat ground
    pass
...
@configclass
class Dog2EnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the dog1 locomotion velocity-tracking environment."""

    # Scene settings
    scene: Dog2SceneCfg = Dog2SceneCfg(num_envs=32000, env_spacing=2.5)
    # Basic settings
    ...
    def __post_init__(self):
        ...
        # simulation settings
        self.sim.dt = 0.005  # 200fps
        self.sim.render_interval = self.decimation
        self.sim.physics_material = self.scene.terrain.physics_material
        # PhysX settings optimized for your setup
        self.sim.physx.solver_type = 1  # TGS solver for better performance
        self.sim.physx.use_gpu = True  # Ensure GPU acceleration
        self.sim.physx.gpu_max_rigid_patch_count = 8 * 2**16  # Adjusted for smaller env count
        self.sim.physx.gpu_max_rigid_contact_count = 2**24 # 8M contacts
        self.sim.physx.gpu_found_lost_pairs_capacity = 2**22  # 2M pairs
        self.sim.physx.gpu_found_lost_aggregate_pairs_capacity = 2**26  # 32M aggregate pairs
        self.sim.physx.gpu_total_aggregate_pairs_capacity = 2**20  # 512k total aggregate pairs
...
@configclass
class Dog2EnvCfg_PLAY(Dog2EnvCfg):
    """Configuration for playing/testing the trained dog1 agent."""
